...
const FUNDING_OWNER_FORBIDDEN_MESSAGE = '??????????????諛몃마??λ???????⑥ル럯?????????????????????깅즽????????놁졄.';
const AUTH_REQUIRED_MESSAGE = '?????????? ?????袁ㅻ쇀??????饔낅떽?????傭??????影?력???????戮?Ĳ??';
...
const assertFundingProjectOwner = async (fundingId, user) => {
  ...
  if (rows.length === 0) {
    throw createHttpError(404, '?????????諛몃마??λ???????⑥ル럯????饔낅떽???????????????깅즽????????놁졄.');
  }
...
const assertFundingDraftOwner = async (draftId, user, expectedFundingId = null) => {
  ...
  if (rows.length === 0) {
    throw createHttpError(404, '?????諛몃마嶺뚮??껆빊?????????諛몃마??λ???????⑥ル럯????饔낅떽???????????????깅즽????????놁졄.');
  }
  ...
  if (
    ...
    throw createHttpError(403, '?????밸븶???????????밸븶?ⓥ뮧?????怨쀫뎐????????곕츣????????밸븶筌믩끃異???????????밸븶?癲??????낆젵.');
  }
...
const parseReviewStringArrayField = (value, fieldName) => {
  const invalidMessage = `${fieldName} ??轅붽틓???????곷뾼????????? ????????????놁졄.`;
...
const REVIEW_RATING_ERROR_MESSAGE = 'rating?? 0~5 ?????0.5 ??????됰뼱????????????꿔꺂??????';
...
const mapFundingReview = (review) => {
  ...
  return {
    ...
    userId: writerId,
    user_id: writerId,
    writerNickname: review.writer_nickname || '??ъ구',
...
const mapFundingReviewComment = (comment) => {
  ...
  return {
    ...
    userId: writerId,
    user_id: writerId,
    writerNickname: comment.writer_nickname || '??ъ구',
...
const storeUploadedFile = async (file, folder, ownerId = 'anonymous') => {
  ...
  if (strictS3) {
    throw new Error('S3 ??????????嚥싲갭큔?????????諛몃마????꿔꺂??????');
  }
...
const saveAgreement = async (req, res) => {
  ...
  if (
    ...
    return res.status(400).json({
      status: 400,
      message: '?????諛몃마????????饔낅떽????ш낄?뉔뇡?????????ㅼ뒭????????깅짆?????꿔꺂??????',
    });
  }
  ...
  try {
    ...
    return res.status(200).json({
      ...
      message: '??????? ??????ㅼ뒭??????ル늉?? ?????얜궙??뷀떐??궰????????????놁졄.',
    });
  } catch (error) {
    return res.status(500).json({
      status: 500,
      message: '??????? ??????ㅼ뒭?? ?????????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};

// ?????????諛몃마??λ???????⑥ル럯???????諛몃마嶺뚮??껆빊???????熬곣뫖利???????癰궽블뀯???癲?????
const createFundingDraft = async (req, res) => {
  ...
  if (!breweryId && requestedFundingId === null) {
    ...
    if (imageUrls && !Array.isArray(imageUrls) && typeof imageUrls !== 'string') {
      return res.status(400).json({
        status: 400,
        message: '?????????饔낅떽???? ?饔낅떽????ш낄?뉔뇡?꾩땡沃섏쥓?????????ㅼ굣塋????ル늉??????????? ????????????놁졄.',
      });
    }
    ...
    if (imageUrls && normalizeFundingImageUrlsInput(imageUrls).length > 5) {
      return res.status(400).json({
        status: 400,
        message: '?????????饔낅떽???????饔낅떽?????壤? 5????ル늉??????? ??關?쒎첎?嫄??怨몃룯?????????????????놁졄.',
      });
    }
    ...
    if (tags && !Array.isArray(tags) && typeof tags !== 'string') {
      return res.status(400).json({
        status: 400,
        message: '??棺堉?뤃????????⑤슢堉?繹먮냱踰 ??????ㅼ굣塋????ル늉??????????? ????????????놁졄.',
      });
    }
  ...
  try {
    ...
    if (requestedFundingId !== null) {
      ...
      if (existingLinkedDraft) {
        ...
        return res.status(200).json({
          ...
          message: '?????곕츣????????????밸븶?ⓥ뮧?????怨쀫뎐???????밸븶筌믩끃異??????????녾낮?녔틦?쀂???????????낆젵.',
        });
      }
    ...
    return res.status(201).json({
      ...payload,
      draft,
      message: '?????????諛몃마??λ???????⑥ル럯?????ル늉?? ?????諛몃마嶺뚮??껆빊?????얜궙??뷀떐??궰????????????놁졄.',
    });
  } catch (error) {
    console.error(error);
    ...
    return res.status(500).json({
      status: 500,
      message: '?????????諛몃마??λ???????⑥ル럯???????諛몃마嶺뚮??껆빊?????????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};

// ?????諛몃마嶺뚮??껆빊?????????諛몃마??λ???????⑥ル럯??????癰궽블뀯??(?????諛몃마嶺뚮??껆빊????????癰궽블뀯???)
  ...
  try {
    ...
    if (!updatedDraft) {
      return res.status(404).json({
        status: 404,
        message: '?????獄쏅챶留덌┼???猿녿퉲?????????獄쏅챶留??貫????????Β?ル윲????耀붾굝????????????????源낆┰?????????곸죩.',
      });
    }
    ...
    return res.status(200).json({
      ...payload,
      draft: updatedDraft,
      message: '?????獄쏅챶留덌┼???猿녿퉲?????????獄쏅챶留??貫????????Β?ル윲??????ル뒌?? ?????곌떽釉붾???????????',
    });
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({
        status: 404,
        message: '?????諛몃마嶺뚮??껆빊?????????諛몃마??λ???????⑥ル럯????饔낅떽???????????????깅즽????????놁졄.',
      });
    }
    ...
    return res.status(200).json({
      ...
      message: '?????諛몃마嶺뚮??껆빊?????????諛몃마??λ???????⑥ル럯?????ル늉?? ????癰궽블뀯???????????',
    });
  } catch (error) {
    console.error(error);
    ...
    return res.status(500).json({
      status: 500,
      message: '?????諛몃마嶺뚮??껆빊?????????諛몃마??λ???????⑥ル럯??????癰궽블뀯???????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};
...
const saveBasicInfo = async (req, res) => {
  ...
  if (!category || !title || !mainIngredient || !alcoholPercentage || !summary) {
    return res.status(400).json({
      status: 400,
      message: '?????諛몃마???????????轅붽틓?????????饔낅떽????ш낄?뉔뇡?????????ㅼ굣塋??????깅짆?????꿔꺂??????',
    });
  }
  ...
  try {
    ...
    return res.status(200).json({
      ...
      progressRate: draft.progress_rate,
      updatedAt: draft.updated_at,
      message: '?????????轅붽틓???????먯땡沃섃넄?곈툣?????쎛 ?????얜궙??뷀떐??궰????????????놁졄.',
    });
  } catch (error) {
    console.error(error);
    ...
    return res.status(500).json({
      status: 500,
      message: '?????????轅붽틓????????????????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};
// ?饔낅떽????ш낄?뉔뇡??猷???癲ル슢?롨린?????산뭐??& ???μ떜媛?걫???????
const saveSchedule = async (req, res) => {
  ...
  if (
    ...
    return res.status(400).json({
      status: 400,
      message: '???ル봿???? ?????? ?????밸븶?ⓥ뮧?????怨쀫뎐??????????援온??????⑤챷竊???ル봿????????癲?? ???????????낆젵.',
    });
  }
  ...
  if (Number.isNaN(deliveryDate.getTime())) {
    return res.status(400).json({
      status: 400,
      message: '????傭?????썹땟戮녹???????꿔꺂???影?우Ŀ????轅붽틓???????곷뾼????????? ????????????놁졄.',
    });
  }
  ...
  if (deliveryDate <= endDate) {
    return res.status(400).json({
      status: 400,
      message: '????傭?????썹땟戮녹???????꿔꺂???影?우Ŀ??? ??????????닿틢?????????밸븶????????꿔꺂??????',
    });
  }
  ...
  try {
    ...
    return res.status(200).json({
      ...
      progressRate: draft.progress_rate,
      updatedAt: draft.updated_at,
      message: '?饔낅떽????ш낄?뉔뇡??猷??????亦낃콛?????????????μ떜媛?걫?????????얜궙??뷀떐??궰????????????놁졄.',
    });
  } catch (error) {
    console.error(error);
    ...
    return res.status(500).json({
      status: 500,
      message: '?饔낅떽????ш낄?뉔뇡??猷??????亦낃콛?????????????μ떜媛?걫????????????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};
...
const saveLegalInfo = async (req, res) => {
  ...
  if (normalizedRawMaterials.length === 0) {
    return res.status(400).json({
      status: 400,
      message: '?饔낅떽?????壤??1???????傭?????????猷몄굣??? ??????ㅼ굣塋??????깅짆?????꿔꺂??????',
    });
  }
  ...
  try {
    ...
    return res.status(200).json({
      ...
      progressRate: draft.progress_rate,
      updatedAt: draft.updated_at,
      message: '?癲????얜?逾????????????轅붽틓???????먯땡沃섃넄?곈툣?????쎛 ?????얜궙??뷀떐??궰????????????놁졄.',
    });
  } catch (error) {
    console.error(error);
    ...
    return res.status(500).json({
      status: 500,
      message: '?癲????얜?逾????????????轅붽틓????????????????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};
...
const saveTasteProfile = async (req, res) => {
  ...
  if (isOutOfRange) {
    return res.status(400).json({
      status: 400,
      message: '?饔낅떽?????????꿔꺂??????0?????뼿???100 ?????????ル늉?????????깅짆?????꿔꺂??????',
    });
  }
  ...
  try {
    ...
    return res.status(200).json({
      ...
      progressRate: draft.progress_rate,
      updatedAt: draft.updated_at,
      message: '?饔낅떽??????????轅붽틓???????먯땡沃섃넄?곈툣?????쎛 ?????얜궙??뷀떐??궰????????????놁졄.',
    });
  } catch (error) {
    console.error(error);
    ...
    return res.status(500).json({
      status: 500,
      message: '?饔낅떽?????????????????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};

// ?????諛몃마??λ???????⑥ル럯????鶯ㅺ동????????轅붽틓???????????API
const savePlan = async (req, res) => {
  ...
  if (
    ...
    return res.status(400).json({
      status: 400,
      message: '?????諛몃마??λ???????⑥ル럯????鶯ㅺ동????????????ㅼ굣塋????ル늉??????????? ????????????놁졄.',
    });
  }
  ...
  try {
    ...
    return res.status(200).json({
      ...
      progressRate: draft.progress_rate,
      updatedAt: draft.updated_at,
      message: '?????諛몃마??λ???????⑥ル럯????鶯ㅺ동????????轅붽틓???????먯땡沃섃넄?곈툣?????쎛 ?????얜궙??뷀떐??궰????????????놁졄.',
    });
  } catch (error) {
    console.error(error);
    ...
    return res.status(500).json({
      status: 500,
      message: '?????諛몃마??λ???????⑥ル럯????鶯ㅺ동????????轅붽틓????????????????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};
...
const saveBreweryInfo = async (req, res) => {
  ...
  if (normalizedAccountNumber.length < 8) {
    return res.status(400).json({
      status: 400,
      message: '??鶯ㅺ동?????띾?繹먮굝??????????紐껓폏???????8??????????傭??????깅즿??????꿔꺂??????',
    });
  }
  ...
  try {
    ...
    return res.status(200).json({
      ...
      progressRate: draft.progress_rate,
      updatedAt: draft.updated_at,
      message: '?饔낅떽?????????轅붽틓?????????????轅붽틓???????먯땡沃섃넄?곈툣?????쎛 ?????얜궙??뷀떐??궰????????????놁졄.',
    });
  } catch (error) {
    return res.status(500).json({
      status: 500,
      message: '?饔낅떽?????????轅붽틓?????????????轅붽틓????????????????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};

// ?????諛몃마????熬곣뫖利????????댄뱼??1: ????筌믨퉭堉?????轅붽틓???????????⑥ル럯???????⑥ロ꺘??
const loadBreweryInfo = async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({
      status: 500,
      message: '????筌믨퉭堉?????轅붽틓??????????怨쀫뮡????????????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};
const uploadFundingDraftFile = async (req, res) => {
  ...
  if (!allowedFileTypes.includes(fileType)) {
    return res.status(400).json({
      status: 400,
      message: '?饔낅떽??????雅?퍔瑗?땟??????????源녾텛? ??????猿딅빟 ??????????????????戮?Ĳ??',
    });
  }
  ...
  try {
    ...
    if (fileType === 'PROJECT_IMAGE' || fileType === 'FUNDING_IMAGE') {
      ...
      if (currentImageUrls.length >= 5) {
        return res.status(400).json({
          status: 400,
          message: '?????????耀붾굝????????耀붾굝??????鶯? 5?????ル뒌??????? ?????롮쾸?椰???⑤챶猷??????????????????곸죩.',
        });
      }
    ...
    return res.status(500).json({
      status: 500,
      message: '?????????????????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};
//?????諛몃마????熬곣뫖利????????댄뱼??3: ??????????쇰뮛?筌믡꺃?????轅붽틓?????믊븐꽫?API
const generateFundingDraftAiImage = async (req, res) => {
  ...
  if (!draftId || isNaN(Number(draftId))) {
    return res.status(400).json({
      status: 400,
      message: '?????諛몃마嶺뚮??껆빊?????????諛몃마??λ???????⑥ル럯??ID????ル늉?? ??????? ????????????놁졄.',
    });
  }
  ...
  try {
    ...
    if (currentImageUrls.length >= 5) {
      return res.status(400).json({
        status: 400,
        message: '?????諛몃마??λ???????⑥ル럯???????????饔낅떽???????饔낅떽?????壤? 5?耀붿빓???????? ??關?쒎첎?嫄??怨몃룯?????????????????놁졄.',
      });
    }
    ...
    if (!aiPayload.name) {
      return res.status(400).json({
        status: 400,
        message: 'AI ?????饔낅떽???? ???熬곣뫖利????????熬곻퐢夷ⓨ떳?벬??????諛몃마??λ???????⑥ル럯?????????????諛몃마????꿔꺂??????',
      });
    }
    ...
    if (aiResult.aiStatus === 'prompt_only') {
      return res.status(200).json({
        status: 200,
        message: aiResult.message || 'AI ?????饔낅떽???? ???熬곣뫖利?????????諛몃마?????熬곻퐢夷①뇾??????????썹땟戮녹?????????????',
        data: {
    ...
    return res.status(status).json({
      status,
      message: status === 500
        ? 'AI ?????饔낅떽???? ???熬곣뫖利????????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.'
        : error.message,
    });
  }
};
...
const verifyPhoneForFundingDraft = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      phoneVerified: draft.phone_verified,
      updatedAt: draft.updated_at,
      message: '??????????쇰뮛?筌믡꺃?????轅붽틓?????믊븐꽫???????諛몃마???????????',
    });
  } catch (error) {
    return res.status(500).json({
      status: 500,
      message: '????????轅붽틓?????믊븐꽫??饔낅떽???壤굿?戮㏐광???????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};
//?????諛몃마????熬곣뫖利????????댄뱼??4: ?????源낅뼌???鶯ㅺ동?????띾????轅붽틓?????믊븐꽫???????깅뤊??????
const verifyAccountForFundingDraft = async (req, res) => {
  ...
  if (
    ...
    return res.status(400).json({
      status: 400,
      verified: false,
      message: '?????깅떔???壤굿???잍를???꿔꺂?????????轅붽틓??熬곥끇釉????????⑤챷竊????觀????꿔꺂?????',
    });
  }
  ...
  try {
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({
        status: 404,
        message: '?????밸븶筌믩끃異?????????????밸븶?ⓥ뮧?????怨쀫뎐????轅붽틓?????????????욱룏???????낆젵.',
      });
    }
    ...
    return res.status(200).json({
      ...
      accountVerified: draft.account_verified,
      updatedAt: draft.updated_at,
      message: '?????깅떔???壤굿???잍를???꿔꺂????癒κ섶???????밸븶???癲???????',
    });
  } catch (error) {
    return res.status(error.status || 500).json({
      status: error.status || 500,
      message: error.message || '?????깅떔???壤굿???잍를???꿔꺂????癒κ섶??轅붽틓??影?뽧걤??????癲ル슢캉???쭍??????⑤챷逾???ル봿?? ??ш끽維뽳쭩?좊쐪筌먲퐢?????????????낆젵.',
      error: error.message,
    });
  }
};
...
const requestBankAccountVerification = async (req, res) => {
  ...
  if (!bankName || !accountNumber || !normalizedAccountNumber || !accountHolder) {
    return res.status(400).json({
      status: 400,
      message: '??鶯ㅺ동?????띾????轅붽틓?????믊븐꽫?????嫄??癲ル슢흮??밤뀋??????????? ????????????놁졄.',
    });
  }
  ...
  try {
    ...
    const response = {
      ...
      message: '??鶯ㅺ동?????띾????轅붽틓?????믊븐꽫?????嫄???????熬곣뫖利????????????',
    };

    if (shouldExposeBankVerificationCode()) {
      response.verificationCode = verificationCode;
      response.devMessage = '???嚥싲갭큔?源??1??????????????⑤슢堉??곕??????????꾨굴??????熬곥끇????ヂ??????源녾텛? ????ル늉??沃섃뫖荑????癲??嶺???怨????轅붽틓???????????轅붽틓?????믊븐꽫?????????댄뱼????????戮?Ĳ??';
    }
    ...
    return res.status(500).json({
      status: 500,
      message: '??鶯ㅺ동?????띾????轅붽틓?????믊븐꽫?????嫄??????熬곣뫖利????????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};
...
const confirmBankAccountVerification = async (req, res) => {
  ...
  if (!bankName || !accountNumber || !normalizedAccountNumber || !accountHolder || !verificationCode) {
    return res.status(400).json({
      status: 400,
      message: '??鶯ㅺ동?????띾????轅붽틓?????믊븐꽫???轅붽틓????????????嫄??癲ル슢흮??밤뀋??????????? ????????????놁졄.',
    });
  }
  ...
  try {
    ...
    if (verificationResult.rows.length === 0) {
      return res.status(400).json({
        status: 400,
        message: '??鶯ㅺ동?????띾????轅붽틓?????믊븐꽫?????嫄?????饔낅떽???????????????쒋닪????饔낅떽?????傭??????????',
      });
    }
    ...
    return res.status(200).json({
      ...
      message: '??鶯ㅺ동?????띾????轅붽틓?????믊븐꽫???????諛몃마???????????',
    });
  } catch (error) {
    return res.status(500).json({
      status: 500,
      message: '??鶯ㅺ동?????띾????轅붽틓?????믊븐꽫???轅붽틓?????????????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};

// ???汝????????????關?쒎첎?嫄?濚밸Þ猿???轅붽틓?????믊븐꽫?????녾컯????욱렱嶺??????? ????
const saveNotices = async (req, res) => {
  ...
  if (
    ...
    return res.status(400).json({
      status: 400,
      message: '?????諛몃마?????????????饔낅떽????ш낄?뉔뇡?????????ㅼ굣塋??????깅짆?????꿔꺂??????',
    });
  }
...
const uploadDocument = async (req, res) => {
  ...
  if (!allowedMimeTypes.includes(file.mimetype)) {
    return res.status(400).json({
      status: 400,
      message: '?饔낅떽??????雅?퍔瑗?땟??????????源녾텛? ??????猿딅빟 ???????轅붽틓???????곷뾼??????戮?Ĳ??',
    });
  }
  ...
  try {
    ...
    return res.status(201).json({
      ...
      message: isAllRequiredDocumentsUploaded
        ? '?????諛몃마?????꿔꺂??틝???堉????ル늉?? ?饔낅떽????ш낄?뉔뇡???????????꿔꺂?????온???????諛몃마??λ???????⑥ル럯?????????100% ?????諛몃마???????????'
        : '?????諛몃마?????꿔꺂??틝???堉????ル늉?? ????????꿔꺂?????온????????????놁졄.',
    });
  } catch (error) {
    console.error(error);
    ...
    return res.status(500).json({
      status: 500,
      message: '?????諛몃마?????꿔꺂??틝???堉?????????????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};
...
//???????⑥ル럯??癲??嶺??????꿔꺂?????(????雅??????댄뱼??!!)
// ?????????諛몃마??λ???????⑥ル럯?????꿔꺂?????+ ???嚥싲갭큔?源????????棺堉?뤃??????룸챷?? ???熬곣뫖利???
const submitFundingDraft = async (req, res) => {
  ...
  try {
    ...
    if (Number(draft.progress_rate) < 100) {
      return res.status(400).json({
        status: 400,
        message: '?????諛몃마????轅붽틓?????????饔낅떽????ш낄?뉔뇡?????????ㅼ굣塋???????꿔꺂????????????????????놁졄.',
      });
    }
    ...
    if (!hasAllRequiredDocuments) {
      return res.status(400).json({
        status: 400,
        message: '?????諛몃마????轅붽틓?????믊븐꽫????꿔꺂??틝???堉???饔낅떽????ш낄?뉔뇡?????????????嶺?????꿔꺂????????????????????놁졄.',
    ...
    try {
      ...
      if (!Number.isInteger(Number(breweryUserId)) || Number(breweryUserId) <= 0) {
        ...
        return res.status(400).json({
          status: 400,
          message: '????筌믨퉭堉??????????轅붽틓???????먯땡沃섃넄?곈툣?????쎛 ??????깅즿?????꿔꺂??????????????깅즽????????놁졄.',
        });
      }
      ...
      return res.status(200).json({
        ...
        message: '?????????諛몃마??λ???????⑥ル럯?????ル늉?? ???꿔꺂??????????????????棺堉?뤃??????룸챷???????熬곣뫖利????????????',
      });
    } catch (error) {
      await client.query('ROLLBACK');
      throw error;
    ...
    return res.status(500).json({
      status: 500,
      message: '?????????諛몃마??λ???????⑥ル럯?????꿔꺂??????????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};

// ?????諛몃마嶺뚮??껆빊??????傭?끆????筌먐삳펶 ???怨쀫뮡????
const getFundingDraft = async (req, res) => {
  ...
  if (!draftId || isNaN(Number(draftId))) {
    return res.status(400).json({
      status: 400,
      message: '?????諛몃마嶺뚮??껆빊???????怨쀫뮡????????嫄??癲ル슢흮??밤뀋??????????? ????????????놁졄.',
    });
  }
  ...
  try {
    ...
    return res.status(500).json({
      status: 500,
      message: '?????諛몃마嶺뚮??껆빊?????????諛몃마??λ???????⑥ル럯?????怨쀫뮡?????????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};
...
const getFundingDraftByFundingId = async (req, res) => {
  ...
  try {
    ...
    if (!draft) {
      return res.status(404).json({
        status: 404,
        message: '?????怨뺤르????????諛몃마嶺뚮??껆빊?????????諛몃마??λ???????⑥ル럯????饔낅떽???????????????깅즽????????놁졄.',
      });
    }
    ...
    return res.status(500).json({
      status: 500,
      message: '?????????諛몃마??λ???????⑥ル럯???????곸궔???????????????怨쀫뮡?????????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};

// ?????諛몃마嶺뚮??껆빊?????饔낅떽????ш낄?뉔뇡?꾩땡沃섏쥓??????怨쀫뮡????
const getFundingDraftList = async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({
      status: 500,
      message: '?????諛몃마嶺뚮??껆빊?????饔낅떽????ш낄?뉔뇡?꾩땡沃섏쥓??????怨쀫뮡?????????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};
  ...
  try {
    ...
    if (isManagementDraft) {
      return res.status(409).json({
        status: 409,
        message: '??關?쒎첎?嫄??怨몃룯?????꿔꺂????????????????????곸궔?????녾컯?????????諛몃마嶺뚮??껆빊????? ?????????????깅즽????????놁졄.',
        data: {
          draftId: Number(draft.draft_id),
          fundingId: draft.funding_id === null || draft.funding_id === undefined
            ? null
    ...
    return res.status(200).json({
      draftId: result.rows[0].draft_id,
      message: '?????諛몃마嶺뚮??껆빊?????????諛몃마??λ???????⑥ル럯?????ル늉?? ?????????????',
    });
  } catch (error) {
    return res.status(500).json({
      status: 500,
      message: '?????諛몃마嶺뚮??껆빊?????????????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};

// ?????諛몃마??λ???????⑥ル럯??????븍툖??野껊갭????????濚밸Ŧ援잏댆?
const getFundingDraftPreview = async (req, res) => {
  ...
  if (!draftId || isNaN(Number(draftId))) {
    return res.status(400).json({
      status: 400,
      message: '????븍툖??野껊갭????????濚밸Ŧ援잏댆?????嫄??癲ル슢흮??밤뀋??????????? ????????????놁졄.',
    });
  }
  ...
  try {
    ...
    return res.status(500).json({
      status: 500,
      message: '?????諛몃마??λ???????⑥ル럯??????븍툖??野껊갭????????濚밸Ŧ援잏댆????怨쀫뮡?????????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};
...
const updateFundingProject = async (req, res) => {
  ...
  if (
    ...
    return res.status(400).json({
      status: 400,
      message: '?饔낅떽????ш낄?뉔뇡??猷??????亦낃콛??????????????ㅼ굣塋????ル늉??????????? ????????????놁졄.',
    });
  }
  ...
  if (imageUrls !== undefined && !Array.isArray(imageUrls) && typeof imageUrls !== 'string') {
    return res.status(400).json({
      status: 400,
      message: '?????????耀붾굝????? ?耀붾굝?????????붾눀?袁⑸븸亦껋꼷伊??????????쇨덫櫻?????ル뒌??????????? ?????????????곸죩.',
    });
  }
  ...
  try {
    ...
    return res.status(500).json({
      status: 500,
      message: '?????????諛몃마??λ???????⑥ル럯??????癰궽블뀯???????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};

// ???????⑥ル럯??癲??嶺????饔낅떽????ш낄?뉔뇡?꾩땡沃섏쥓??????怨쀫뮡????
...
const getFundingStats = async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({
      status: 500,
      message: '????????????怨쀫뮡?????????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};
...
const getFundingDetail = async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({
      status: 500,
      message: '?????????紐껊괘?????怨쀫뮡?????????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};
...
const getFundingIntro = async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({
      status: 500,
      message: '?????諛몃마??λ???????⑥ル럯??????????怨쀫뮡?????????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};
...
const getBreweryLogs = async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({
      status: 500,
      message: '????筌믨퉭堉???? ???怨쀫뮡?????????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};
...
const createBreweryLog = async (req, res) => {
  ...
  if (!stage || !title || !content) {
    return res.status(400).json({
      status: 400,
      message: '????筌믨퉭堉???? ???꿔꺂??씙?猷곗뫀?????????⑤뜤?????????ㅼ굣塋??????깅짆?????꿔꺂??????',
    });
  }
  ...
  if (!allowedStages.includes(stage)) {
    return res.status(400).json({
      status: 400,
      message: '????筌믨퉭堉???饔낅떽?????嶺뚮ㅎ?????傭?끆???????汝??繹?? ??????? ????????????놁졄.',
    });
  }
  ...
  try {
    ...
    if (videoUrl && !breweryLogColumns.has('video_url')) {
      return res.status(500).json({
        status: 500,
        message: '????戮깅뼑???? ?????몃뱥癲?URL ???????β뼯爰?????????밸븶??볧돯?DB???????⑤뜪???? ?????源낅돹??????',
    ...
    return res.status(500).json({
      status: 500,
      message: '????筌믨퉭堉???? ??關?쒎첎?嫄??怨몃룯???????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};
...
const updateBreweryLog = async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({
      status: 500,
      message: '????筌믨퉭堉???? ????癰궽블뀯???????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};
...
const deleteBreweryLog = async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({
      status: 500,
      message: '????筌믨퉭堉???? ?????????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};
//qna ?饔낅떽????ш낄?뉔뇡?꾩땡沃섏쥓??????怨쀫뮡????
const getFundingQuestions = async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({
      status: 500,
      message: 'Q&A ?饔낅떽????ш낄?뉔뇡?꾩땡沃섏쥓??????怨쀫뮡?????????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};

//qna ?饔낅떽?????????彛??關?쒎첎?嫄??怨몃룯??
...
const createFundingReply = async (req, res) => {
  ...
  if (!fundingId || isNaN(Number(fundingId)) || !questionId || isNaN(Number(questionId))) {
    return res.status(404).json({ status: 404, message: '?饔낅떽?????????彛???????????諛몃마??λ???????⑥ル럯????饔낅떽???????????????깅즽????????놁졄.' });
  }

  if (!content || typeof content !== 'string' || content.trim() === '') {
    return res.status(400).json({ status: 400, message: '??? ??????ㅼ굣塋????ル늉??????????? ????????????놁졄.' });
  }
  ...
  try {
    ...
    return res.status(500).json({
      status: 500,
      message: '??? ??關?쒎첎?嫄??怨몃룯???????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};
...
const likeFundingQuestion = async (req, res) => {
  ...
  try {
    ...
    return res.status(201).json({
      ...
      message: 'Q&A ??????????? ??關?쒎첎?嫄??怨몃룯?????????????놁졄.',
    });
  } catch (error) {
    return res.status(500).json({
      status: 500,
      message: 'Q&A ????????????關?쒎첎?嫄??怨몃룯???????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};
...
const unlikeFundingQuestion = async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({
      status: 500,
      message: 'Q&A ?????????????????????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};
...
const getFundingReviews = async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({
      status: 500,
      message: '????熬곥끇????饔낅떽????ш낄?뉔뇡?꾩땡沃섏쥓??????怨쀫뮡?????????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};
...
const getFundingReviewDetail = async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({
      status: 500,
      message: '????熬곥끇????????紐껊괘?????怨쀫뮡?????????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};
...
const likeFundingReview = async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({
      status: 500,
      message: '????熬곥끇???????????????關?쒎첎?嫄??怨몃룯???????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};
...
const unlikeFundingReview = async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({
      status: 500,
      message: '????熬곥끇????????????????????????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};
...
const getSupportOptions = async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({
      status: 500,
      message: '?????諛몃마??????????怨쀫뮡?????????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};
...
const FUNDING_ORDER_CLOSED_MESSAGE = '?????살꺎????????袁⑸즴筌?㈀????????밸븶??????????욱룏???????낆젵.';
...
const createFundingOrder = async (req, res) => {
  ...
  if (!adultVerified) {
    return res.status(400).json({
      status: 400,
      message: '?????⑹름?癲ル슢?????????諛몃마???????熬곻퐢夷????關?쒎첎?嫄?濚밸Þ猿???轅붽틓?????믊븐꽫???????諛몃마????꿔꺂??????',
    });
  }
  ...
  try {
    ...
    if (!Number.isFinite(pricePerBottle) || pricePerBottle <= 0) {
      return res.status(400).json({
        status: 400,
        message: '?????諛몃마???????????ル늉????棺堉?뤃?筌롡뫖沅?????嚥싲갭큔?????꿔꺂????????? ????????????놁졄.',
      });
    }
    ...
    const orderName = `${funding.title || '?? ??'} ${bottleCount}?`;
    ...
    return res.status(500).json({
      status: 500,
      message: '?????諛몃마???????⑹름?癲ル슢??늴우?????熬곣뫖利????????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};
...
const createFundingInquiry = (req, res) => {
  ...
  return res.status(201).json({
    fundingId: Number(fundingId),
    inquiryId: 21,
    message: '???嶺???????ㅼ굣?? ??關?쒎첎?嫄??怨몃룯???????????',
  });
};


//?????댄뱼??4: ?????????怨룹??? ?饔낅떽??????ヂ?塋딆늺?????怨쀫뮡????
...
const getFundingReports = async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({
      status: 500,
      message: '?????節뗭젔???饔낅떽????ш낄?뉔뇡?꾩땡沃섏쥓??????怨쀫뮡?????????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};
...
const createFundingReview = async (req, res) => {
  ...
  try {
    ...
    if (!createdReview) {
      return res.status(500).json({
        status: 500,
        message: '????熬곥끇?????關?쒎첎?嫄??怨몃룯????棺堉?뤃???????リ틖濾???轅붽틓?????????????????깅즽????????놁졄.',
      });
    }
...
const updateFundingReview = async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({
      status: 500,
      message: '????熬곥끇???????癰궽블뀯???????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};
...
const getFundingReviewComments = async (req, res) => {
  ...
  if (
    ...
    return res.status(400).json({
      status: 400,
      message: '????熬곥끇?????? ?饔낅떽????ш낄?뉔뇡?꾩땡沃섏쥓???????嫄??癲ル슢흮??밤뀋??????????? ????????????놁졄.',
    });
  }
...
const unlikeFundingProject = async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({
      status: 500,
      message: '????????⑤뜪???????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};
...
const getBreweryLogComments = async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({
      status: 500,
      message: '????筌믨퉭堉???? ??? ?饔낅떽????ш낄?뉔뇡?꾩땡沃섏쥓??????怨쀫뮡?????????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.',
      error: error.message,
    });
  }
};
...
const createFundingReviewStable = async (req, res) => {
  ...
  try {
    ...
    const createdReview = result.rows[0];
    if (!createdReview) {
      throw createHttpError(500, '????熬곥끇?????關?쒎첎?嫄??怨몃룯????棺堉?뤃???????リ틖濾???轅붽틓?????????????????깅즽????????놁졄.');
    }
    ...
    return sendFundingReviewError(res, error, '????熬곥끇?????關?쒎첎?嫄??怨몃룯???????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.');
  }
};
...
const updateFundingReviewStable = async (req, res) => {
  ...
  try {
    ...
    return sendFundingReviewError(res, error, '????熬곥끇???????癰궽블뀯???????꿔꺂??틝???彛???????ㅼ굣?????ル늉?? ????썹땟戮녹??醫딆맚嶺뚮㉡???????????????놁졄.');
  }
};
...
module.exports = {
  ...
  savePlan,
  saveBreweryInfo,
  loadBreweryInfo, //?????諛몃마????熬곣뫖利???????댄뱼??1 ?????뼿???
  uploadFundingDraftFile,
  generateFundingDraftAiImage,
  verifyPhoneForFundingDraft,
  verifyAccountForFundingDraft,//?????댄뱼??4
  requestBankAccountVerification,
  confirmBankAccountVerification,
  saveNotices,
  uploadDocument,
  submitFundingDraft, //?????諛몃마??λ???????⑥ル럯????꿔꺂?????
  getFundingDraft,    //?????諛몃마嶺뚮??껆빊??????傭?끆????筌먐삳펶 ???怨쀫뮡????
  getFundingDraftByFundingId,
  getFundingDraftList, //?????諛몃마嶺뚮??껆빊?????饔낅떽????ш낄?뉔뇡?꾩땡沃섏쥓??????怨쀫뮡????
  deleteFundingDraft,  //?????諛몃마嶺뚮??껆빊????????
  getFundingDraftPreview,  //?????諛몃마??λ???????⑥ル럯??????븍툖??野껊갭????????濚밸Ŧ援잏댆?
  updateFundingProject, // ????????????????諛몃마??λ???????⑥ル럯??????癰궽블뀯??
  getFundingList,
  getFundingStats,
  getFundingDetail,
  getFundingIntro,
