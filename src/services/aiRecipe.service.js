...
const getFromAiServer = async (path, query = {}) => {
  const AI_SERVER_BASE_URL = process.env.AI_SERVER_BASE_URL;
  ...
  try {
    ...
    const connectionError = new Error('AI 서버에 연결할 수 없습니다.');
    connectionError.status = 502;
    throw connectionError;
  } finally {
    clearTimeout(timeout);
...
const getIngredientRegion = async ({ main_ingredient }) => (
  getFromAiServer('/api/recipe/ingredient-region', {
